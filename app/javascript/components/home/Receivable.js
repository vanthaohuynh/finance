...
const Receivable = ({ receivable }) => {
  ...
  const total = receivable.reduce((acc, cur) => acc + cur.revenue_total, 0);
  ...
  const renderTransactions = () => {
    const sortedAccounts = [...receivable].sort((a, b) => (a.account_num > b.account_num ? 1 : -1));
    return (
      <Box
        sx={{
          height: 350,
        ...
        <div className="eventList">
          <Typography
            sx={{
              ...
              fontWeight: 'bold',
              fontVariantCaps: 'all-small-caps',
              backgroundColor: '#74e093',
            }}
            variant="button"
            display="block"
            align="center"
          >
            Amounts Receivable for Current Studies
          </Typography>
          {renderTransactions()}
...
Receivable.propTypes = {
  receivable: PropTypes.arrayOf(PropTypes.shape({
